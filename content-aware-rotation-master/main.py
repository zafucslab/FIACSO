...
# 新建输出文件夹
output_folder_new = 'D:\\Network model file\\code\\beauty\\3finish_content-aware-rotation-master\\output_rotation_images'
...
def image_rotation(img_path, angle, M=90, lambda_r=1e2, lambda_b=1e8, lambda_l=1e2, lambda_s=1):
    ...
    result_img = Image.fromarray(img_result)
    result_img.show()

    # 创建一个新的文件夹用于保存输出图像
    output_folder = 'D:\\Network model file\\code\\beauty\\3finish_content-aware-rotation-master\\output_rotation_images'
    ...
    # 在原始文件名前加上 "output_"
    new_filename = "output_" + original_filename
...
if __name__ == '__main__':
    img_path = 'D:\\Network model file\\code\\beauty\\3finish_content-aware-rotation-master\\example\\s2x.jpg'
    angle = 10
